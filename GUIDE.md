...
- отдельной компоненты (`choose-correct-answer`, `fill-in-the-blank`, `match-pairs`, `yes-no-questions`);
  ...
  "tasks": [
    {
      "id": "yn1",
      "title": "Опционально",
      "textMarkdown": "Текстовый блок в формате **Markdown**. Можно использовать заголовки, списки, *выделение*.",
      "questions": [
        { "id": "q1", "text": "Утверждение 1", "correctAnswer": true },
        { "id": "q2", "text": "Утверждение 2", "correctAnswer": false }
      ]
    }
  ]
}
...
Правила:
- В каждом `task` поле `textMarkdown` — один текст задания в формате Markdown.
- В каждом `task` минимум 1 вопрос в `questions`.
- `correctAnswer: true` означает правильный ответ `ДА`.
- `correctAnswer: false` означает правильный ответ `НЕТ`.
...
[
  ...
  },
  {
    ...
    "tasks": [
      {
        "id": "yn1",
        "textMarkdown": "Короткий текст для чтения.",
        "questions": [
          { "id": "q1", "text": "Утверждение верно.", "correctAnswer": true },
          { "id": "q2", "text": "Утверждение неверно.", "correctAnswer": false }
        ]
      }
    ]
  }
...
- Корневой тип: массив.
- Каждый элемент массива должен быть валидным объектом одного из 4 типов.
...
- Для `yes-no-questions`: у каждого `task` есть непустое `textMarkdown` и `questions`, а в вопросах есть булево `correctAnswer`.
...
6) yes-no-questions:
   - tasks[] -> { id, textMarkdown, questions[] }
   - questions[] -> { id, text, correctAnswer }
   - correctAnswer: true для "ДА", false для "НЕТ"
7) Итоговый JSON должен парситься без ошибок.
```
